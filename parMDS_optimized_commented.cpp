...
postProcessIt(const VRP &vrp, std::vector<std::vector<node_t>> &final_routes, weight_t &minCost) {
  ...
  // FIX: resize vector before parallel writes to avoid unsafe push_back() to eliminate data races during parallel writes. (kapil)
  postprocessed_final_routes.resize(final_routes.size());
...
#pragma omp parallel for
  for (unsigned zzz = 0; zzz < final_routes.size(); ++zzz) {
    ...
    if (postprocessed_route3_cost > postprocessed_route2_cost) {
      postprocessed_final_routes[zzz] = postprocessed_route2;  // thread-safe indexed write to avoid concurrent push_back corruption. (kapil)
    }
    // postprocessed_route3_cost is lower
    else {
      postprocessed_final_routes[zzz] = postprocessed_route3;  // thread-safe indexed write to avoid concurrent push_back corruption. (kapil)
    }
  }
...
// FIX: removed nested OpenMP parallelism to avoid oversubscription and thread management overhead. (kapil)
    for (unsigned jj = 1; jj < final_routes[ii].size(); ++jj) {
      curr_route_cost += vrp.get_dist(final_routes[ii][jj - 1], final_routes[ii][jj]);
    }
    curr_route_cost += vrp.get_dist(DEPOT, final_routes[ii][final_routes[ii].size() - 1]);
...
// FIX: Each thread now uses a private MST copy and private best solution to eliminate shared-memory races. (kapil)
#pragma omp parallel num_threads(PARLIMIT)
{
  weight_t localMinCost = INT_MAX * 1.0f;
  std::vector<std::vector<node_t>> localMinRoute;

  // FIX: replaced shared rand() usage with per-thread randomization to reduce contention. (kapil)
  //thread_local std::mt19937 rng(std::random_device{}());
  

#pragma omp for schedule(dynamic) // Added dynamic scheduling for improved workload balancing. (kapil)
  for (int i = 0; i < 100000; i++) {
    auto localMstCopy = mstG;
    std::mt19937 rng(i + omp_get_thread_num() * 100000); // Added iteration and thread-dependent RNG seed for better search diversity. (kapil)
    // RANDOMIZE THE ADJ LIST OF MST
    for (auto &list : localMstCopy) {
      std::shuffle(list.begin(), list.end(), rng);
    }

    std::vector<int> singleRoute;

    std::vector<bool> visited(localMstCopy.size(), false);
    visited[0] = true;

    ShortCircutTour(localMstCopy, visited, 0, singleRoute);

    auto aRoutes = convertToVrpRoutes(vrp, singleRoute);

    auto aCostRoute = calCost(vrp, aRoutes);

    // FIX: thread-local best update reduces contention on global minimum variables. (kapil)
    if (aCostRoute.first < localMinCost) {
      localMinCost = aCostRoute.first;
      localMinRoute = aCostRoute.second;
    }
  }

  // FIX: protected global update using critical section to avoid race conditions on minCost/minRoute. (kapil)
#pragma omp critical
  {
    if (localMinCost < minCost) {
      minCost = localMinCost;
      minRoute = localMinRoute;
    }
  }
}
